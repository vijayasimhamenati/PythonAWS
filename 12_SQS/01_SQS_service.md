...
4. **Delete:** Once processing is successful, the consumer *must* call the `DeleteMessage` API. If it fails to do this, the message will eventually reappear in the queue and be processed again by another consumer.
...
* **Message Size:** Maximum of **256 KB** (1,024 kilobytes is a misstatement in the transcript; the hard limit is 256 KB. To send larger payloads, you use the SQS Extended Client Library which stores the payload in S3 and sends a reference link in the queue).
* **Retention Period:** Messages are kept in the queue for **4 days** by default, up to a maximum of **14 days**.
* **Delivery Guarantee:** **At-Least-Once Delivery**. Because it is a highly distributed system, SQS may occasionally deliver the same message more than once. Your consumer application must be *idempotent* (able to handle duplicate messages safely without corrupting data).
